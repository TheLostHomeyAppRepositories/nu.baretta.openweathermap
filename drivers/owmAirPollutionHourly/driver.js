...
class owmAirPollutionHourlyDriver extends Homey.Driver {
    onPair(session) {
        this.log("onPair()");
        this.settingsData = {
            "hours": "0",
   ...
   */
    async onPairListDevices(session) {
        ...
        let devices = [];
        if (!this.settingsData.hours){
            this.settingsData.hours = 0;
        }
        let locations = this.homey.drivers.getDriver('owmAirPollutionCurrent').getDevices();
        for (let i=0; i<locations.length; i++){
            devices.push(
            {
                name: locations[i].getName() + " +" + this.settingsData.hours + "h",
                data: {
                    id: this.getUIID(),
                    locationId: locations[i].getData().id
                },
                settings:{
                    hours: parseInt(this.settingsData.hours),
...
module.exports = owmAirPollutionHourlyDriver;
