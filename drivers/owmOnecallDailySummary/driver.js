...
class owmOnecallDailySummaryDriver extends Homey.Driver {

    getExistingAPIKey(){
        let devices = this.homey.drivers.getDriver('owmOnecallDailySummary').getDevices();
   ...
   */
    async onPairListDevices(session) {
        ...
        if (!this.settingsData.GEOlocationCity){
            ...
            let deviceName = "Lat:" + this.settingsData["lat"] + ' Lon:' + this.settingsData["lon"] + " " + this.settingsData.days + "d";
            ...
            try{
                ...
                for (let i=0; i<geoData.length; i++){
                    let deviceName = geoData[i].name+', '+geoData[i].country+', '+geoData[i].state + " " + this.settingsData.days + "d";
                    // let deviceId = crypto.randomBytes(16).toString('hex');
                    let deviceId = this.getUIID();
                    let device = {
                        name: deviceName,
...
module.exports = owmOnecallDailySummaryDriver;
