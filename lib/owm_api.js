...
function getOnecallURL(settings, onecall40Type = 'current') {
    // OncallAPI only supports lan/lot, no city geocoding
    let coordinateQuery = 'lat=' + settings['lat'] + '&lon=' + settings['lon'];
    let url = "";
...
function getOnecall40URL(settings, onecall40Type = 'current') {
    // OncallAPI only supports lan/lot, no city geocoding
    let coordinateQuery = 'lat=' + settings['lat'] + '&lon=' + settings['lon'];
    let url = "";
    switch (onecall40Type){
        case "current":
            url = '/data/4.0/onecall/current?';
            break;
        case "hourly":
            url = '/data/4.0/onecall/timeline/1h?';
            break;
        case "daily":
            url = '/data/4.0/onecall/timeline/1day?';
            break;
    }
    url = url + coordinateQuery + '&units=' + settings['units'] + '&lang=' + settings['language'] + '&mode=json&APPID=' + settings["APIKey"];
    return url;
...
function getOnecall40AlertURL(settings, alertId) {
    // OncallAPI only supports lan/lot, no city geocoding
    let url = "";
    url = '/data/4.0/onecall/alert/' + alertId + '?';
    url = url + 'APPID=' + settings["APIKey"];
    return url;
};
